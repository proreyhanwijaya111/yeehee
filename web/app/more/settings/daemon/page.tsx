...
function buildInstallScript(supaUrl: string, supaKey: string, show: boolean): string {
  ...
  return [
    `# yeehee daemon — install + run (PowerShell)`,
    `$ErrorActionPreference = 'Stop'`,
    `$Dest = "$HOME\\yeehee-daemon"`,
    ``,
    `Write-Host "[yeehee] Cloning repo..."`,
    `if (Test-Path $Dest) { Set-Location $Dest; git pull } else { git clone ${REPO_URL} $Dest; Set-Location $Dest }`,
    ``,
    `Write-Host "[yeehee] Creating venv..."`,
    `python -m venv .venv`,
    `& .\\.venv\\Scripts\\Activate.ps1`,
    ``,
    `Write-Host "[yeehee] Installing deps..."`,
    `pip install --upgrade pip --quiet`,
    `pip install -r requirements.txt --quiet`,
    `pip install supabase httpx psutil --quiet`,
    ``,
    `Write-Host "[yeehee] Writing .env..."`,
    `@"`,
    `SUPABASE_URL=${url}`,
    ...
    ``,
    `Write-Host "[yeehee] Starting daemon..."`,
...
function buildUpdateScript(): string {
  return [
    ...
    `& .\\.venv\\Scripts\\Activate.ps1`,
    `pip install -r requirements.txt --quiet`,
    `Write-Host "[yeehee] Restart daemon (Ctrl+C window lama, run lagi)"`,
    `python -m daemon.main`,
  ].join('\n')
}
...
function buildServiceScript(): string {
  return [
    `# Install daemon sebagai Windows Service (auto-start)`,
    `# Run as Administrator`,
    `winget install NSSM.NSSM`,
    ``,
    `$DaemonDir  = "$HOME\\yeehee-daemon"`,
    `$PythonExe  = "$DaemonDir\\.venv\\Scripts\\python.exe"`,
    `$Arguments  = "-m daemon.main"`,
    ``,
    `nssm install yeehee-daemon $PythonExe $Arguments`,
    ...
    `Write-Host "[yeehee] Service installed & started ✅"`,
  ].join('\n')
}
